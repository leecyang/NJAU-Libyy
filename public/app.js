...
function credentialActive() { return state.me?.credential?.credential_status === "ACTIVE" && state.me?.credential?.setup_required !== true; }
...
async function bindForm(event) { event.preventDefault(); await busy($("button[type=submit]", event.target), async () => { try { await api("/api/v1/credentials/bind", { method: "POST", body: JSON.stringify(formData(event.target)) }); event.target.reset(); await refreshMe(); toast("统一认证任务已启动，请使用新版界面完成可能的短信验证"); } catch (error) { toast(error.message, true); } }); }
