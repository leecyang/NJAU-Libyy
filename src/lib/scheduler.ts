...
export async function runScheduler(env: AppEnv): Promise<void> {
  ...
  await budget.run("accept-reservation-members", async () => {
    await submitPendingMemberAcceptanceTasks(env, now, schedulerLimit(env, "SCHEDULER_SYNC_LIMIT", 5, 20));
  }, 2500);
...
async function submitReadyReservationTasks(env: AppEnv, now: number, limit = 2): Promise<void> {
  ...
  for (const task of tasks.results) {
    ...
    try {
      ...
      await enqueueMemberAcceptanceTasks(env, task.owner_user_id, String(official.id), members, {
        roomId: submitted.room.id,
        date: task.target_date,
        startTime: task.start_time,
        endTime: task.end_time,
      });
      if (members.some((member) => member.localUserId) && env.OFFICIAL_GATEWAY) {
        await env.OFFICIAL_GATEWAY.enqueue({
          kind: "MEMBER_ACCEPTANCE",
          lane: "WRITE",
          ownerUserId: task.owner_user_id,
          dedupeKey: `member-accept:${reservationId}:all`,
          payload: { reservationId, officialReservationId: String(official.id) },
          priority: 4,
          maxAttempts: 1,
        });
      }
